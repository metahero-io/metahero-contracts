...
describe('MetaheroLPMForUniswapV2', () => {
  ...
  before(async () => {
    ...
    const factory = await deployContract('PancakeFactory', owner.address);

    wrappedNative = await deployContract('WrappedNativeMock');
    ...
    stableCoin = await deployContract('ERC20Mock');
  ...
  const createBeforeHook = (
    ...
    before(async () => {
      lpManager = await deployContract('MetaheroLPMForUniswapV2');
